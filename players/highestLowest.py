...
class HighestLowest(Player):
    """
    Author: @BeanGal
    Description: Sorts hand when received in descending order, then plays cards in that order
    """

    def __init__(self):
        super().__init__("Highest -> Lowest")

    def set_hand(self, hand: Hand):
        hand.cards.sort(key=lambda card: card.value, reverse=True)
        self.hand = hand
